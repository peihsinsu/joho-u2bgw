...
//cost api 5 units quota
addBroadCast = function (auth,youtube,next) {
...
//cost api 5 units quota
addStream = function (auth,youtube,next) {
  var reqStream = {
    part: 'snippet,cdn',
    resource: {
      snippet: {
 ...
 */
listBroadCast = function(auth,youtube,status,next){
  var listArg = {
...
processStream = function (auth,youtube,rtspSrc,vid,streamConfig){
  var fmpg = new ffmpeg({
      ...
      .on('start', function(it){
        var retryCnt = 0;
        if( streamConfig.status == 0 && retryCnt < 10 ){
          setInterval(function(){
            listStream(auth,youtube,streamConfig.sid,function(err,stream){
              ...
              for(var i=0;i<stream.items.length;i++){
                ...
                console.log('transit:',retryCnt,item.status,sstatus==streamStatus[1]);
...
getAuth(serviceAccount,sunnyToken,function(err,auth){
  ...
  listBroadCast(auth,youtube,'upcoming',function(err,cfg){
    ...
    for(var i=0;i<cfg.items.length;i++){
      ...
      if(liveFlag) {
        if (cfg.items[i].status.lifeCycleStatus == streamStatus[1] ||
          ...
          if (cfg.items[i].snippet.title.indexOf('ITRI') > -1) {
            liveFlag = false;
            vid = cfg.items[i].id;
    ...
    if(liveFlag){
      doLiveStream(auth,youtube,'rtsp://104.155.214.170/GACWPNS291O6U46L0SC4');
    } else {
      listStream(auth,youtube,sid,function(err,streams){
        ...
        processStream(auth,youtube,'rtsp://104.155.214.170/GACWPNS291O6U46L0SC4',vid,streamConfig);
      });
    }
  });
