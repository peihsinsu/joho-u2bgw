...
var ffmpeg = require('fluent-ffmpeg');
  ...
  transitIt = function(auth, client, status, next){
  ...
  //googleapis.discover('youtube', 'v3').execute(function(err, client){
    return getAccessToken(auth, function(){
      ...
      reqBroadcast = {
        ...
        resource: {
          snippet: {
            scheduledStartTime: moment(new Date('2015', '10', '15', '23')).format('YYYY-MM-DDThh:mm:ss.sZ'),
            scheduledEndTime: moment(new Date('2015', '10', '16', '08')).format('YYYY-MM-DDThh:mm:ss.sZ'),
            title: 'first-live'
          },
          status: {
...
//'http://104.155.214.170/HLS/GACWPNS291O6U46L0SC4/HLS.m3u'
              }
            ).addInputOption('-rtsp_transport tcp')
                .withVideoCodec('libx264')
             //.withAudioCodec('libfaac').withAudioBitrate('128k').withAudioChannels(1).withSize('426x240')
             .withAudioFrequency(44100).withFps(30).toFormat('flv').addOptions(['-g 1', '-force_key_frames 2'])
              ...
              return youtube.liveStreams.list({
                ...
                //return transitIt(auth, client, 'testing', function(err, test){
                  return transitIt(auth, client, 'live', function(err, live){
                    if (live) {
            ...
            }).writeToStream(streamUrl);//.stream(streamUrl);
          });
        });
      });
    });
